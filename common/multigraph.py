...
class MultiGraph(object):
    def __init__(self, name = 'Untitled'):
        self._vertices = [] # list of v_id
        self._edges = {} # e_id => tuple of (a_id, b_id)
        self._neighbors = {} # v_id => list of e_id
        self._colors = {} # (e_id, a_id) => color
        self._positions = {} # v_id => (x, y)
        self.name = name
        
        # q is the set of all available id for edges
        q = Queue.PriorityQueue()
        q.queue = range(1, MAX_NUMBER_OF_EDGES + 1, 1)
...
######################################################
    def add_vertex(self, v_id, position = None):
    ...
    def add_edge(self, a, b, color = None):
        ...
        if color == None:
            self.set_color(e_id, a, 0)
            self.set_color(e_id, b, 0)
        else:
    ...
    def from_json(cls, content):
        ...
        for vertex in data['vertices']:
            ...
            graph._positions[v_id] = (int(vertex['x']), int(vertex['y']))
...
######################################################
    ...
    def smooth_vertex(self, v_id):
        ...
        self.remove_edge(e_b)
        self.remove_edge(e_c)
    ...
    def delete_vertex(self, v_id):
        eid_list = self._neighbors[v_id]

        length = len(self._neighbors[v_id])
        while len(self._neighbors[v_id]) > 0:
            eid = self._neighbors[v_id][0]
    ...
    def set_color(self, e_id, a, c):
        if self.get_edge(e_id) and a in self.get_edge(e_id):
