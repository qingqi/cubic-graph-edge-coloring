class Path(object):
    '''
    This class represents a Kempe path in a cubic graph.
    '''
    ...
    def swap_colors(self, c1, c2):
        '''
        Perform (c1, c2) color exchange on all internal vertices on the path
        '''
    ...
    def invert_colors(self):
        '''
        Perform (c1, c2) color exchange on all internal vertices on the path,
        where c1, c2 is the two color on the path.
        '''
        self.c1, self.c2 = self.c2, self.c1
        for i in self._vertices:
            v = self._graph.get_vertex(i)
    ...
    def move_one_step(self):
        '''
        Move the variable on the Kempe path to the next edge along the path
        '''
    ...
    def move_steps(self, num = 1):
        '''
        Move the variable on the Kempe path multiple times to the next edge along the path
        '''
    ...
    def variable(self):
        '''
        Get the variable contained in this Kempe path
        '''
        if self._closed != True:
            return None
...
def create_path(multigraph, va, c1, c2):
    '''
        this method searches a c1 - c2 kempe path start from vertex va and with color c1
